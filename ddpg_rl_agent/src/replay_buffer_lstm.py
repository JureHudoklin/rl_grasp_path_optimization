...
class Buffer:
    ...
    def record(self, obs_tuple):
        ...
        self.old_state_buffer[index] = obs_tuple[0]
    ...
    def sample_buffer(self):
        ...
        self.update(old_state_batch, state_batch, action_batch,
                    reward_batch, next_state_batch)

    def save_buffer(self, episode):
        np.savez(os.path.join(self.model_dir, f'memory_buffer{episode}'),
                 arr_1=self.old_state_buffer,
                 arr_2=self.state_buffer,
                 arr_3=self.action_buffer,
                 arr_4=self.reward_buffer,
                 arr_5=self.next_state_buffer)
        return

    def load_buffer(self, id):
        npzfile = np.load(os.path.join(self.model_dir, f'memory_buffer{id}.npz'))
        self.old_state_buffer = npzfile['arr_1']
        self.state_buffer = npzfile['arr_2']
        self.action_buffer = npzfile['arr_3']
        self.reward_buffer = npzfile['arr_4']
        self.next_state_buffer = npzfile['arr_5']
...
class Agent():
    ...
    def sample_action(self, old_state, state, tr = True):
        state_series = tf.stack((old_state, state), axis=1)
    ...
    @tf.function
    def update(self, old_state_batch,  state_batch, action_batch, reward_batch, next_state_batch):
        # Training and updating Actor & Critic networks.
        # See Pseudo Code.
        # TRAIN CRITIC NETWORK

        old_to_state = tf.stack([old_state_batch, state_batch], axis=1)
        state_to_next_state = tf.stack([state_batch, next_state_batch], axis=1)

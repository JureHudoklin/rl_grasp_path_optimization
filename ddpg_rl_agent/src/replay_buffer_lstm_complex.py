...
import numpy as np
from network_lstm_complex import *
import pickle
...
class Buffer:
    def __init__(self, num_actions, num_states, buffer_capacity = 100000, batch_size = 64, model_dir = None):
        ...
        self.old_2_state_buffer = np.zeros((self.buffer_capacity, num_states))
        self.old_state_buffer = np.zeros((self.buffer_capacity, num_states))
    ...
    def record(self, obs_tuple):
        ...
        self.old_2_state_buffer[index] = obs_tuple[0]
        self.old_state_buffer[index] = obs_tuple[1]
        self.state_buffer[index] = obs_tuple[2]
    ...
    def sample_buffer(self):
        ...
        # Convert to tensors
        old_2_state_batch = tf.convert_to_tensor(
            self.old_2_state_batch[batch_indices])
        old_state_batch = tf.convert_to_tensor(
            self.old_state_batch[batch_indices])
        ...
        self.update(old_2_state_batch, old_state_batch, state_batch, action_batch,
                    reward_batch, next_state_batch)

    def save_buffer(self, episode):
        np.savez(os.path.join(self.model_dir, f'memory_buffer{episode}'),
                 arr_1=self.old_2_state_buffer,
                 arr_2=self.old_state_buffer,
                 arr_3=self.state_buffer,
                 arr_4=self.action_buffer,
                 arr_5=self.reward_buffer,
                 arr_6=self.next_state_buffer)
        return

    def load_buffer(self, id):
        npzfile = np.load(os.path.join(self.model_dir, f'memory_buffer{id}.npz'))
        self.old_2_state_buffer = npzfile['arr_1']
        self.old_state_buffer = npzfile['arr_2']
        self.state_buffer = npzfile['arr_3']
        self.action_buffer = npzfile['arr_4']
        self.reward_buffer = npzfile['arr_5']
        self.next_state_buffer = npzfile['arr_6']
...
class Agent():
    def __init__(self, num_actions, num_states, lower_bound, upper_bound,
     ...
     model_dir = None):
        ...
        self.actor_optimizer = tf.keras.optimizers.Adam(
            learning_rate=learning_rate_actor, decay = 0.00001)
        self.critic_optimizer = tf.keras.optimizers.Adam(
            learning_rate=learning_rate_critic, decay=0.00001)
    ...
    def sample_action(self, old_2_state,old_state, state, tr = True):
        state_series = tf.stack((old_2_state, old_state, state), axis=1)

        sampled_actions = tf.squeeze(
            self.actor.model(state_series, training=False))
    ...
    @tf.function
    def update(self, old_2_state_batch, old_state_batch,  state_batch, action_batch, reward_batch, next_state_batch):
        # Training and updating Actor & Critic networks.
        # See Pseudo Code.
        # TRAIN CRITIC NETWORK
        old_to_state = tf.stack(
            [old_2_state_batch, old_state_batch, state_batch], axis=1)
        state_to_next_state = tf.stack(
            [old_state_batch, state_batch, next_state_batch], axis=1)

        print(old_to_state.shape, "state")

        with tf.GradientTape() as tape:
            target_actions = self.target_actor.model(
                state_to_next_state, training=False)
            y = reward_batch + self.gamma * self.target_critic.model(
                [state_to_next_state, target_actions], training=True
            )
            critic_value = self.critic.model(
                [old_to_state, action_batch], training=True)
        ...
        # TRAIN ACTOR NETWORK
        with tf.GradientTape() as tape:
            actions = self.actor.model(old_to_state, training=True)
            critic_value = self.critic.model(
                [old_to_state, actions], training=False)
    ...
    def learn(self):
        ...
        # Convert to tensors
        old_2_state_batch = tf.convert_to_tensor(
            self.buffer.old_2_state_buffer[batch_indices])
        old_state_batch = tf.convert_to_tensor(self.buffer.old_state_buffer[batch_indices])

...
import os
from replay_buffer_lstm_complex import Agent
import numpy as np
...
SAVE_DIR = "/home/jure/reinforcement_ws/src/ddpg_rl_agent/src"
M_NUM = 1
...
if __name__ == "__main__":
    ...
    model_dir = "/home/jure/reinforcement_ws/src/ddpg_rl_agent/src/checkpoints_lstm_complex/20211204-040738"
    ...
    while episode_number < num_of_episodes:
        ...
        states.append(state)
        old_2_state = state
        old_2_state_tf = tf.expand_dims(tf.convert_to_tensor(state), 0)
        old_state = state
        ...
        while True:
            ...
            action = ddpg_agent.sample_action(
                old_2_state_tf, old_state_tf, tf_state, tr=False)[0]
            ...
            state = new_state
            old_2_state = old_state
            old_2_state_tf = old_state_tf
            old_state = state
            old_state_tf = tf.expand_dims(tf.convert_to_tensor(state), 0)
            states.append(state)
            if done:
        ...
        if episode_number % 1 == 0:
            np.save(SAVE_DIR + f"/{M_NUM}/CoppeliaSim_{episode_number}",
                    np.array(states))
            states = []

...
class MotionPlanner(object):
    ...
    def update_state_cb(self, msg):
        self.force_torque = np.array([msg.data[3], msg.data[4], msg.data[5], msg.data[0], msg.data[1], msg.data[2]])
        return None
    ...
    def calculate_reward(self):
        ...
        reward = -(abs(self.force_torque[3]) + abs(self.force_torque[4]) + abs(self.force_torque[5]))
        return reward

    def calculate_reward_v3(self, torque, force):
        force = np.array(force)
        torque = np.array(torque)
        print(force, torque, " FORCE TORQUE")
        model_r = 0.032
        coef_friction = 0.5
    ...
    def env_reset_cb(self, msg):
        ...
        # try:
        #     self.move_group.clear_pose_targets()
        #     self.move_group.go(new_pose, wait=True)
        # except:
        #     rospy.sleep(1)
        #     self.move_group.go(new_pose, wait=True)
        ...
        state = self.calculate_reward_v3(self.force_torque[0:3], self.force_torque[3:6])
    ...
    def env_step_cb(self, msg):
        ...
        state = self.calculate_reward_v3(
            self.force_torque[0:3], self.force_torque[3:6])
        print(state)
    ...
    def move_offset(self, offset):
        ...
        for i in range(1,6):
            rot_axis = np.array([offset[0], offset[2], offset[3]])
            rot_axis = rot_axis/np.linalg.norm(rot_axis)

            offset_matrix = tf.transformations.rotation_matrix(offset[0]/i, rot_axis)
            combined_matrix = np.dot(frame_tf, offset_matrix)
            combined_quat = tf.transformations.quaternion_from_matrix(combined_matrix)

            new_pose = cur_pose
            new_pose.pose.position.x = 0.4#(0.4 - cur_pose.pose.position.x)/4
            new_pose.pose.position.y = 0#(0 - cur_pose.pose.position.y)/4
            new_pose.pose.position.z = 0.4#0.02

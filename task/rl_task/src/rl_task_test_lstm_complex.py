...
class MotionPlanner(object):
    ...
    def env_reset_cb(self, msg):
        ...
        #     #return True
        force = np.array(
            [self.force_torque[3], self.force_torque[4], self.force_torque[5]])
        torque = np.array([self.force_torque[0], self.force_torque[1], self.force_torque[2]])

        state = self.calculate_reward_v3(torque, force)
        rospy.logwarn("-----------------------------------------")
